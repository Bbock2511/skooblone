    ...
    return (
        <SafeAreaView style={styles.container}>
                ...
                renderItem={(item) => {
                    return (
                        <View style={{flex: 1, alignContent: 'center', justifyContent: 'center'}}>
                            <Text>{item.name == undefined ? item.name = "Nada": item.name}</Text>
                        </View>
                    )
                }}
                renderEmptyData={() => {
            ...
            <TouchableOpacity style={styles.button} onPress={() => setShowModal(true)}>
                <Text style={{fontSize: 32, textAlign: 'center', textAlignVertical: 'center', color: '#efefef'}}>+</Text>
            </TouchableOpacity>

            <AddBookAgendaModal show={showModal} handleClose={handleShowModal} selectedDay={selectedDate} refresh={refreshingModal} setRefresh={setRefreshingModal}/>
        </SafeAreaView>
    )
}
