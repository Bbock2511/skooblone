...
const Signup = ({ navigation }) => {
    ...
    const onSubmit = async (data) => {
        ...
        .then((userCredential) => {
            addDatabase(db, data)
            setUserAction({ email: data.email, uid: userCredential.user.uid }, dispatch)
        }).catch((error) => {
            ...
            if (error.code == 'auth/user-not-found') {
                ...
                Alert.alert('Email já cadastrado', 'Esse email já está cadastrado no sistema');
            }
        })
        .finally(() => {
        ...
        <View style={styles.container}>
            <View style={styles.card}>
                ...
                <TouchableOpacity onPress={() => navigation.navigate('Login')}>
                    <Text style={{color: '#517EF5', textDecorationLine: 'underline', marginTop: 5}}>Já possuo conta</Text>
                </TouchableOpacity>
            </View>
        </View>
    )
